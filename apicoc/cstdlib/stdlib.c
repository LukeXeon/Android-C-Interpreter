...
void StdlibItoa(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) {
	ReturnValue->Val->Pointer = _itoa(Parser->pc, Param[0]->Val->Integer, Param[1]->Val->Pointer,
		Param[2]->Val->Integer);
}
